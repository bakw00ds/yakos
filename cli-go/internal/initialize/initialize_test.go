...
// TestRun_WithGate_FailsExplicitly_ButStillWritesBaseProject asserts
// `yakos init --with-gate` fails loudly instead of silently succeeding.
// Same shape as the --multi-dev bug above: the Go port doesn't implement
// pre-push gate installation (Phase 1 scope); it used to print an
// advisory to stderr and still return a nil error, so
// `yakos init --with-gate foo && next-step` would run next-step even
// though the gate was never installed. The base project (real, useful
// work) must still be written; only the exit status/error changes.
func TestRun_WithGate_FailsExplicitly_ButStillWritesBaseProject(t *testing.T) {
	proj := newGitRepo(t)
	cfg := baseConfig(t, "withgateproj", proj)
	cfg.WithGate = true

	res, err := Run(cfg)
	if err == nil {
		t.Fatal("expected non-nil error for --with-gate (gate installation not ported); got nil")
	}
	if !strings.Contains(err.Error(), "not ported") {
		t.Errorf("expected 'not ported' in error; got: %v", err)
	}
	...
	// The base project is still real, useful work: it must be written
	// despite the gate-installation gap.
	if res == nil {
		t.Fatal("expected non-nil Result even on --with-gate error")
	}
	controlDir := filepath.Join(cfg.HomeDir, "agent-control", "withgateproj")
	if _, statErr := os.Stat(filepath.Join(controlDir, ".gitignore")); statErr != nil {
		t.Errorf("expected base project files still written despite --with-gate error: %v", statErr)
	}
}
