...
check_dark_code() {
    ...
    while IFS= read -r f; do
        ...
        case "$base" in
            ...
            agent.sh|cost.sh|memory.sh|session.sh|project-config.sh|migrate.sh|plugin.sh|teach.sh|soul.sh|retro.sh|skill.sh|compact.sh|checkpoint.sh|kanban.sh|env.sh) continue ;;
        esac
